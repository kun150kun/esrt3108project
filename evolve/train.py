...
def dnn_evolve_train(dnn,mnist,x_image,loss_sequence0,loss_fitness,x,labels,sess,generation=10):
    ...
    for i in range(generation):
        ...
        for k in range(i0,dnnl):
            ...
            l1 = list()
            for j in range(200):
                t.append(sess.run(accuracy,feed_dict={
                                x: m0.test.images[j*50:(j+1)*50],
                                labels: m0.test.labels[j*50:(j+1)*50], keep_prob: 1.0}))
                l1.append(sess.run(lll,feed_dict={
                                x: m0.test.images[j*50:(j+1)*50],
                                labels: m0.test.labels[j*50:(j+1)*50], keep_prob: 1.0}))
            test_accuracy = np.mean(t)
            lost.append(np.mean(l1,axis = 0))
            dnn_fitness.append([test_accuracy,duration,num_parameter,variance])
            dnn_fitness[index].extend(lost[index])
            print dnn_fitness[index],lost[index]
            index = index + 1
        if i != 9:
            dnn = ga.dnn_add(dnn,lost,3,4)
    loss_fitness.append([jj / index for jj in np.sum(dnn_fitness,axis = 0)])
    return loss_fitness
